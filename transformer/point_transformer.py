...
from transformer.cpp_wrappers.pointops.functions import pointops
from transformer.utils import calc_ppf_gpu, group_all, point_to_node_partition, get_node_correspondences, index_select, get_node_occlusion_score, to_o3d_pcd
import torch.nn.functional as F
from transformer.ppftransformer import LocalPPFTransformer, PPFTransformer
from modules.geotransformer import GeometricTransformer
...
class RIPointTransformer(nn.Module):
    def __init__(self, blocks=[2, 3, 3, 3, 3], block=RIPointTransformerBlock, c=1, transformer_architecture=None, with_cross_pos_embed=None, factor=1, occ_thres=0.):
        super().__init__()
        self.c = c
        self.num_heads = 4
        self.in_planes, planes = c, [64*factor, 128*factor, 256*factor, 256*factor, 256*factor]
        ...
        self.enc5 = self._make_enc(block, planes[4], blocks[4], self.num_heads, stride=stride[4], nsample=nsample[4], factor=factor)  # N/256

        self.dec5 = self._make_dec(block, planes[4], 2, self.num_heads, nsample=nsample[4], factor=factor, is_head=True)  # fusion p6 and p5
    ...
    def forward(self, s_pxon, t_pxon, src_deformed_pcd):
        feats_list, points_list, normals_list = [], [], []
        ...
        ############################
        # source pcd
        s_p1, s_x1, s_o1, s_n1, s_idx1, s_ppf1, _ = self.enc1([s_p0, s_x0, s_o0, s_n0, None, None, None])           # N
        s_p2, s_x2, s_o2, s_n2, s_idx2, s_ppf2, s_d_idx2 = self.enc2([s_p1, s_x1, s_o1, s_n1, None, None, None])    # N/4
        s_p3, s_x3, s_o3, s_n3, s_idx3, s_ppf3, s_d_idx3 = self.enc3([s_p2, s_x2, s_o2, s_n2, None, None, None])    # N/16
        s_p4, s_x4, s_o4, s_n4, s_idx4, s_ppf4, s_d_idx4 = self.enc4([s_p3, s_x3, s_o3, s_n3, None, None, None])    # N/64
        s_p5, s_x5, s_o5, s_n5, s_idx5, s_ppf5, s_d_idx5 = self.enc5([s_p4, s_x4, s_o4, s_n4, None, None, None])    # N/128
        # target pcd
        ...
        t_p5, t_x5, t_o5, t_n5, t_idx5, t_ppf5, _ = self.enc5([t_p4, t_x4, t_o4, t_n4, None, None, None])

        points_list.append([[t_p1, s_p1], [t_p2, s_p2],[t_p3, s_p3], [t_p4, s_p4], [t_p5, s_p5]])
        normals_list.append([[t_n1, s_n1], [t_n2, s_n2],[t_n3, s_n3], [t_n4, s_n4], [t_n5, s_n5]])
        ###########################
        # global PPF & transformer
        ###########################
        # s_nr, s_pr = group_all(s_n5), group_all(s_p5)
        # s_ppf = calc_ppf_gpu(s_p5, s_n5, s_pr, s_nr)  # (n, nsample, 4)

        # t_nr, t_pr = group_all(t_n5), group_all(t_p5)
        # t_ppf = calc_ppf_gpu(t_p5, t_n5, t_pr, t_nr)   # (n, nsample, 4)

        s_g_x5, t_g_x5 = self.global_transformer(s_p5.unsqueeze(0), t_p5.unsqueeze(0), s_x5.unsqueeze(0), t_x5.unsqueeze(0))
        s_g_x5 = s_g_x5[0]
        t_g_x5 = t_g_x5[0]
        ...
        # source pcd
        s_x5 = self.dec5[1:]([s_p5, self.dec5[0]([s_p5, s_x5, s_o5]), s_o5, s_n5, s_idx5, s_ppf5, None])[1]
        s_x4 = self.dec4[1:]([s_p4, self.dec4[0]([s_p4, s_x4, s_o4], [s_p5, s_x5, s_o5]), s_o4, s_n4, s_idx4, s_ppf4, None])[1]
        ...
        # target pcd
        t_x5 = self.dec5[1:]([t_p5, self.dec5[0]([t_p5, t_x5, t_o5]), t_o5, t_n5, t_idx5, t_ppf5, None])[1]
        t_x4 = self.dec4[1:]([t_p4, self.dec4[0]([t_p4, t_x4, t_o4], [t_p5, t_x5, t_o5]), t_o4, t_n4, t_idx4, t_ppf4, None])[1]
        ...
        feats_list.append([[], [],[t_x3, s_x3], [t_x4, s_x4], [t_x5, s_x5]])        # [t_x1, s_x1]
        # s_d_idx3 = s_d_idx2[s_d_idx3.long()]            # 
        # s_d_idx4 = s_d_idx3[s_d_idx4.long()].long()
        # s_d_idx5 = s_d_idx4[s_d_idx5.long()].long()
        # s_p5 = src_deformed_pcd[s_d_idx5]

        return s_p5, s_g_x5, points_list[0], t_p5, t_g_x5, feats_list[0], normals_list[0]
