...
import torch
import numpy as np
from modules.registration.procrustes import weighted_procrustes
from engine.logger import Logger
from utils.summary_board import SummaryBoard
from utils.open3d import registration_with_ransac_from_correspondences
from utils.registration import (
    evaluate_sparse_correspondences,
    evaluate_correspondences,
    compute_registration_error,
)
from modules.datasets.registration.threedmatch.utils import (
    get_num_fragments,
    get_scene_abbr,
...
def make_parser():
    ...
    parser.add_argument('--benchmark', default='3DLoMatch', help='test benchmark') # choices=['3DMatch', '3DLoMatch'], required=True
    parser.add_argument('--method', default='lgr', help='registration method')  # choices=['lgr', 'ransac', 'svd'], required=True, 
    parser.add_argument('--num_corr', type=int, default=250, help='number of correspondences for registration')
...
def eval_one_epoch(args, cfg, logger):
    ...
    for scene_root in scene_roots:
        ...
        for file_name in file_names:
            ...
            # corr_scores = data_dict['select_corr_scores']
            ...
            if args.method == 'lgr':
                ...
                # estimated_transform= lgr(
                #     torch.from_numpy(ref_corr_points).cuda(),
                #     torch.from_numpy(src_corr_points).cuda(),
                #     torch.from_numpy(corr_scores).cuda(),
                #     torch.from_numpy(data_dict['batch_indices']).cuda()) # data_dict['estimated_transform']
