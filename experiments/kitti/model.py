...
class Registration(nn.Module):
    def __init__(self, cfg):
        ...
        self.fc = nn.Linear(cfg.geotransformer.output_dim, cfg.geotransformer.hidden_dim)
    ...
    def forward(self, data_dict):
        ...
        ref_point_corr_indices, src_point_corr_indices = self.global_transformer(ref_points_c, src_points_c, ref_points_f, src_points_f, transform, data_dict=data_dict)    # , training=self.training
        ...
        # feats_m = feats_list[0]
        ref_feats_f, src_feats_f = feats_f[:ref_length_f], feats_f[ref_length_f:]
        ref_corr_feats = ref_feats_f[ref_point_corr_indices]    # output_dict['ref_corr_feats']
        src_corr_feats = src_feats_f[src_point_corr_indices]    # output_dict['src_corr_feats']
        ...
        if self.training:  
            ...
            _, ref_point_corr_overlap_indices = common_elements_indices(ref_overlap_indices.reshape(-1), ref_point_corr_indices)  # 共同元素在第一/二个tensor中的位置索引
        ...
        if not self.training and renew_fitness_one < self.overlap_threshold:
            p = True
            print(data_dict['ref_frame'], data_dict['src_frame'])
            # print(message1)
            print('****************************************')
        ...
        data_save = {
                    ...
                    # "p0_m": src_points_m,       "p1_m": ref_points_m,
                    # "p0_f_normal": src_normals_f,   "p1_f_normal": ref_normals_f,  # 
                    "gt_transform" :transform, 'global_transform': estimated_transform
                    }
        # visualize(data_save)
        ...
        for patch_step in range(1, self.T):
            ...
            ref_points_state, src_points_state = torch.cat((ref_points_state, ref_corr_points_transform)), torch.cat((src_points_state, src_corr_points_transform))
            ...
            nums = min(src_points_state.shape[0], ref_points_state.shape[0])
            ...
            # data_stage_save = {
                        # "p0_corr": src_nodes_state,  "p1_corr": ref_nodes_state,
                        ...
                        # "p0_points_corr_select": src_corr_points_transform[corr_cla],   # src_points_f,  # 
                        # "p1_points_corr_select": ref_corr_points_transform[corr_cla],   # ref_points_f,  # 
                        # "p0_points_corr_select_normal": src_corr_normals_transform[corr_cla],   # src_normals_f,  # 
                        # "p1_points_corr_select_normal": ref_corr_normals_transform[corr_cla],   # ref_normals_f,  # 
                        # "p0_center": src_action,       "p1_center": ref_action,       # src_points[output_dict['src_select_corr_node_indices']]   ref_points[output_dict['ref_select_corr_node_indices']] ，
            ...
            ref_node_indices_action = get_patch(len(ref_points), ref_points_f, ref_action) # ref_patch_size , ref_action_weight   # ref_points_m
            src_node_indices_action = get_patch(len(src_points), src_points_f, src_action) # src_patch_size , src_action_weight   # src_points_m
            ...
            #     ref_feats, src_feats = ref_feats_c, src_feats_c     #torch.concat((ref_corr_points_transform_feat, ref_feats)) if len(ref_points) <5 else ref_feats, torch.concat((src_corr_points_transform_feat, src_feats)) if len(src_points) <5 else src_feats
            #     ref_points, src_points = ref_points_c, src_points_c     # torch.concat((ref_corr_points_transform, ref_points)) if len(ref_points) <5 else ref_points, torch.concat((src_corr_points_transform, src_points)) if len(src_points) <5 else src_points
            #     print(f'abab_{patch_step}--add') 
            if len(ref_points) <5 or len(src_points)<5 :
                print(f'abab_{patch_step}--break') 
                break
            ref_feats, src_feats = self.fc(ref_feats), self.fc(src_feats)
            # renew_fitness_one_before = renew_fitness_one
            # estimated_transform_before = estimated_transform
            ...
            ref_point_corr_indices, src_point_corr_indices = self.local_transformer(ref_points, src_points, ref_points_f, src_points_f, transform, \
                                                                                    ref_feats, src_feats, ref_feats_f, src_feats_f, transform_dict=None)   #
            ...
            if not self.training:
                ...
                if renew_fitness_one_before-renew_fitness_one > 10:
                    break
            ...
            if not self.training and result_dict[f'RR_{99}'] < 1.0:
                ...
                # visualize(data_save) 
                # else:
                #     print(f'abab_continue-no_{patch_step}')   
                #     visualize(data_save) 
                # print(message1)
                # print(message2)
...
def dbs_cluster(points, weights=None, epsilon=0.25, min_samples=3, nums=None): 
    ...
    for i in range(num_clusters): 
        ...
        # feats_cluster.append(torch.tensor(feats[indices]))
        if weights is not None:
...
def classifier(tgt_keypts, src_keypts, trans=None, weights=None):
    ...
    abs_compatibility = torch.abs(cross_dist)#  * weights.repeat(1, len(weights), 1)
...
def remove_repeat(ref_corr_points_transform, src_corr_points_transform, corr_scores_transform):
    ...
    return src_corr_points_uni, ref_corr_points_uni, corr_scores_uni
#########################################################################
def main():
