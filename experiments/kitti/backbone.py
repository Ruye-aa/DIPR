...
from modules.kpconv import ConvBlock, ResidualBlock, UnaryBlock, LastUnaryBlock, nearest_upsample
...
class KPConvFPN(nn.Module):
    ...
    def forward(self, feats, data_dict):
        ...
        feats_s1 = feats
        feats_s1 = self.encoder1_1(feats_s1, points_list[1], points_list[1], neighbors_list[1])
        feats_s1 = self.encoder1_2(feats_s1, points_list[1], points_list[1], neighbors_list[1])

        feats_s2 = self.encoder2_1(feats_s1, points_list[2], points_list[1], subsampling_list[1])
        feats_s2 = self.encoder2_2(feats_s2, points_list[2], points_list[2], neighbors_list[2])
        feats_s2 = self.encoder2_3(feats_s2, points_list[2], points_list[2], neighbors_list[2])

        feats_s3 = self.encoder3_1(feats_s2, points_list[3], points_list[2], subsampling_list[2])
        feats_s3 = self.encoder3_2(feats_s3, points_list[3], points_list[3], neighbors_list[3])
        feats_s3 = self.encoder3_3(feats_s3, points_list[3], points_list[3], neighbors_list[3])

        feats_s4 = self.encoder4_1(feats_s3, points_list[4], points_list[3], subsampling_list[3])
        feats_s4 = self.encoder4_2(feats_s4, points_list[4], points_list[4], neighbors_list[4])
        feats_s4 = self.encoder4_3(feats_s4, points_list[4], points_list[4], neighbors_list[4])

        feats_s5 = self.encoder5_1(feats_s4, points_list[5], points_list[4], subsampling_list[4])
        feats_s5 = self.encoder5_2(feats_s5, points_list[5], points_list[5], neighbors_list[5])
        feats_s5 = self.encoder5_3(feats_s5, points_list[5], points_list[5], neighbors_list[5])

        latent_s5 = feats_s5
        feats_list.append(feats_s5)

        latent_s4 = nearest_upsample(latent_s5, upsampling_list[4])
        latent_s4 = torch.cat([latent_s4, feats_s4], dim=1)
        latent_s4 = self.decoder4(latent_s4)
        feats_list.append(latent_s4)

        latent_s3 = nearest_upsample(latent_s4, upsampling_list[3])
        latent_s3 = torch.cat([latent_s3, feats_s3], dim=1)
        latent_s3 = self.decoder3(latent_s3)
        feats_list.append(latent_s3)

        latent_s2 = nearest_upsample(latent_s3, upsampling_list[2])
...
class Upsample(nn.Module):
    ...
    def forward(self, feats_s4, feats_s3, data_dict):
        upsampling_list = data_dict['upsampling']
