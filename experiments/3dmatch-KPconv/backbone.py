...
class KPConvFPN(nn.Module):
    def __init__(self, input_dim, output_dim, init_dim, kernel_size, init_radius, init_sigma, group_norm):
        ...
        self.encoder1_1 = ConvBlock(input_dim, init_dim, kernel_size, init_radius, init_sigma, group_norm)  # kpconv, norm, leaky_relu
    ...
    def forward(self, feats, data_dict):
        ...
        feats_s1 = feats                                                                          # (N, 1)
        feats_s1 = self.encoder1_1(feats_s1, points_list[1], points_list[1], neighbors_list[1])   # (N, 64)
        feats_s1 = self.encoder1_2(feats_s1, points_list[1], points_list[1], neighbors_list[1])   # (N, 128)

        feats_s2 = self.encoder2_1(feats_s1, points_list[2], points_list[1], subsampling_list[1]) # (N/4, 128)
        feats_s2 = self.encoder2_2(feats_s2, points_list[2], points_list[2], neighbors_list[2])   # (N/4, 256)
        feats_s2 = self.encoder2_3(feats_s2, points_list[2], points_list[2], neighbors_list[2])   # (N/4, 256)

        feats_s3 = self.encoder3_1(feats_s2, points_list[3], points_list[2], subsampling_list[2]) # (N/8, 256)
        feats_s3 = self.encoder3_2(feats_s3, points_list[3], points_list[3], neighbors_list[3])   # (N/8, 512)
        feats_s3 = self.encoder3_3(feats_s3, points_list[3], points_list[3], neighbors_list[3])   # (N/8, 512)

        feats_s4 = self.encoder4_1(feats_s3, points_list[4], points_list[3], subsampling_list[3]) # (N/16, 512)
        feats_s4 = self.encoder4_2(feats_s4, points_list[4], points_list[4], neighbors_list[4])   # (N/16, 1024)
        feats_s4 = self.encoder4_3(feats_s4, points_list[4], points_list[4], neighbors_list[4])   # (N/16, 1024)
